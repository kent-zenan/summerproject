...
//			return false;
	 ...
	 */
	public void run
	(int maximumSeconds, int maximumShakeTime, boolean tabuEnabled, 
			int shakeBase) {
//	(int maximumSeconds, int maximumShakeTime, boolean tabuEnabled, 
//			int shakeBase, int shakeBonusThreshold) {
		...
		for (int i = 0; i < nw.routesOfShifts.size(); i++) {
			...
			//Stopping Criteria
			eTime.add(Calendar.SECOND, maximumSeconds);
...
//			while (Calendar.getInstance().before(eTime) && shakeTrial < maximumShakeTrial) {
			while (shakeBase + shakeBonus < maximumShakeTime) {
				...
				case 1:
					...
					break;
				case 2:
					if (!insertFromUnassigned(mandatory)) {
						k++;
					} else
						k = 0;
					...
					break;
				case 3:
					if (!insertFromUnassigned(mandatory2)) {
						k++;
					} else
						k = 0;
					...
					break;
				case 4:
					if (!insertFromUnassigned(optional)) {
						k++;
					} else
						k = 0;
...
//					if (shakeBonus > shakeBonusThreshold) {
//						shakeTime = shakeBase;
//						tabuLength = 5;
//						
//					} else {
...
//					}
			...
			mandatory = nw.getPeriodTaskSet(currentShiftIndex, true);
	 ...
	 */
	private boolean insertFromUnassigned(Vector<Task> unassigned) {
