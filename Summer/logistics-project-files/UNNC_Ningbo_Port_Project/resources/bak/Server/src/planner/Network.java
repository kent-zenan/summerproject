...
//		}
	...
	public String outputRoutes() {
		...
		for (int i = 0; i < routesOfShifts.size(); i++) {
			output += "----============= Shift " + i + " with " + routesOfShifts.get(i).size() + " routes =============----\n";
	...
	public int getShiftRoutedTaskCount(int shiftI) {
		...
		for (int j = 0; j < shiftISize; j++) {
			count += routesInShiftI.get(j).taskSet.size();
		}
		return count;
	}
	...
	public String getSolutionObjectiveValues() {
		...
		for (int i = 0; i < psize; i++) {
			...
			res += l + "\t" + e + "\t";
			res += (l / (e + l)) + "\t" + this.getShiftRoutedTaskCount(i) + "\n"; 
			
		}
		
		res += "Avg\t" + total_l + "\t" + total_e + 
				"\t" + (total_l / (total_l + total_e)) + 
				"\t" + this.getTotalRoutedTaskCount() + "\n";
		return res;
	}
