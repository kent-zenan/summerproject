		...
		stop: function(e) {
			...
			for (var i = this.containers.length - 1; i >= 0; i--){
				if(contains(this.containers[i].element[0], this.currentItem[0])) {						
						...
						var a = $("#" + this.containers[i].element[0].id).children();
						
						for(var k = 1; k < a.length; k++) {
							if(this.currentItem[0].id == a.eq(k)[0].id){
								var taskID = this.currentItem[0].id;
								var routeID = a.eq(0)[0].id;
								var sequenceNo = k-1;
								//alert(this.currentItem[0].id);
								//alert(this.containers[i].element[0].id);
								//alert(k-1);
								 var url = "Change1";  
								 var param = "taskID="+ taskID +"&routeID="+routeID
